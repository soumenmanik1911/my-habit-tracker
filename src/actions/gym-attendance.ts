...
export async function updateGymAttendance(date: string, attended: boolean, workoutType?: string, notes?: string) {
  try {
    ...
    const existingRecord = await sql`
      SELECT id FROM GymAttendance WHERE date = ${date}
    `;
    ...
    if (existingRecord.length > 0) {
      ...
      // Insert new record
      result = await sql`
        INSERT INTO GymAttendance (date, attended, workout_type, notes)
        VALUES (${date}, ${attended}, ${workoutType || null}, ${notes || null})
        RETURNING *
      `;
    }
...
export async function getGymStats(days: number = 30): Promise<{
  ...
  try {
    ...
    const stats = await sql`
      ...
      LEFT JOIN GymAttendance ga ON ga.date = date_series.date
    `;
    ...
    const currentStreak = await calculateCurrentGymStreak();

    // Calculate longest streak
    const longestStreak = await calculateLongestGymStreak();
...
async function calculateCurrentGymStreak(): Promise<number> {
  try {
    const today = new Date().toISOString().split('T')[0];
    
    // Check if today has gym attendance recorded
    const todayRecord = await sql`
      SELECT attended FROM GymAttendance WHERE date = ${today}
    `;
    ...
    if (todayRecord.length === 0 || !todayRecord[0].attended) {
      ...
      const yesterdayRecord = await sql`
        SELECT attended FROM GymAttendance WHERE date = ${yesterdayStr}
      `;
    ...
    while (true) {
      ...
      const record = await sql`
        SELECT attended FROM GymAttendance WHERE date = ${dateStr}
      `;
...
async function calculateLongestGymStreak(): Promise<number> {
  try {
    const records = await sql`
