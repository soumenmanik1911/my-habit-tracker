...
export async function createTask(data: {
  ...
  try {
    ...
    const result = await sql`
      ...
      VALUES (${data.title.trim()}, ${data.description || null}, ${data.priority}, ${data.due_date || null}, ${data.category})
      RETURNING *
    `;
...
export async function toggleTask(taskId: number) {
  try {
    ...
    const currentTask = await sql`
      SELECT is_completed FROM tasks WHERE id = ${taskId}
    `;
...
export async function updateTaskDetails(taskId: number, data: {
  ...
  try {
    ...
    if (data.title !== undefined) {
      await sql`
        UPDATE tasks SET title = ${data.title.trim()} WHERE id = ${taskId}
      `;
    }
    if (data.description !== undefined) {
      await sql`
        UPDATE tasks SET description = ${data.description} WHERE id = ${taskId}
      `;
    }
    if (data.priority !== undefined) {
      await sql`
        UPDATE tasks SET priority = ${data.priority} WHERE id = ${taskId}
      `;
    }
    if (data.due_date !== undefined) {
      await sql`
        UPDATE tasks SET due_date = ${data.due_date} WHERE id = ${taskId}
      `;
    }
    if (data.category !== undefined) {
      await sql`
        UPDATE tasks SET category = ${data.category} WHERE id = ${taskId}
      `;
    }
